...
import localSearch.LocalSearchHashCode;
import structure.HashCodeInstance;
import structure.HashCodeSolution;

import java.util.concurrent.TimeUnit;

public class AlgConstructiveLS implements Algorithm<HashCodeInstance> {

	final GRASPLibrary graspLibrary;
	final GRASPBook graspBook;

	public AlgConstructiveLS(GRASPLibrary graspLibrary, GRASPBook graspBook){
	...
	public Result execute(HashCodeInstance hashCodeInstance){
		...
		for(int i=0; i<iterations;i++) {
			...
			double valueConstructive = value;
			...
			if(value<valueConstructive) hashCodeSolutionImprove=hashCodeSolution;
			value = hashCodeSolutionImprove.getObjectiveFunctionValue();
			System.out.println("Iteration BL " + i + " out of " + iterations + ". Value: " + valueConstructive + ". BL: " + value);
			if(value>sol){
				sol = value;
